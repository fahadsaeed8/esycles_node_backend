...
router.post(
  ...
  [
    ...
    body('mobile_number').optional().isMobilePhone().withMessage('Valid mobile number is required'),
    body('password').isLength({ min: 6 }).withMessage('Password must be at least 6 characters'),
    body('confirm_password').custom((value, { req }) => value === req.body.password)
      .withMessage('Passwords do not match'),
    body('role').optional().isIn(['vendor', 'customer', 'admin', 'influencer'])
      .withMessage('Invalid role specified'),
    
    // Address validation
    body('address.street').if(body('role').equals('vendor')).notEmpty().withMessage('Street address is required'),
    body('address.city').if(body('role').equals('vendor')).isMongoId().withMessage('Valid city ID is required'),
    body('address.state').if(body('role').equals('vendor')).isMongoId().withMessage('Valid state ID is required'),
    body('address.country').if(body('role').equals('vendor')).isMongoId().withMessage('Valid country ID is required'),
    body('address.zip_code').if(body('role').equals('vendor')).notEmpty().withMessage('ZIP code is required'),
    
    // Vendor-specific validation
    body('company_info.company_name').if(body('role').equals('vendor')).notEmpty().withMessage('Company name is required'),
    body('company_info.title').if(body('role').equals('vendor')).notEmpty().withMessage('Title is required'),
    ...
    body('agreed_to_terms').isBoolean().withMessage('You must agree to the terms')
  ],
  async (req, res) => {
    ...
    try {
      ...
      if (!agreed_to_terms) {
        return res.status(400).json({ success: false, message: 'Terms agreement is required' });
      }
      ...
      if (role === 'vendor') {
        ...
        if (!city || !state || !country) {
          return res.status(400).json({ success: false, message: 'Invalid address references' });
        }
      }
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const otpEntry = await UserOTP.findOne({ email, otp, is_verified: false }).sort({ created_at: -1 });
      ...
      return res.json({ success: true, message: 'OTP verified and user activated' });
    } catch (err) {
      console.error(err);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user.is_active) {
        return res.status(403).json({ success: false, message: 'User is not verified yet' });
      }
...
router.post('/billing_address', auth, async (req, res) => {
  try {
    ...
    if (!countryExists || !stateExists || !cityExists) {
      return res.status(400).json({ message: 'Invalid country, state, or city' });
    }
